<?php
/**
 * Entr Customizer Notification Section Class.
 *
 * @package Entr
 */

/**
 * Entr_Customizer_Notice_Section class
 */
class Entr_Customizer_Notice_Section extends WP_Customize_Section {
	/**
	...
	public function json() {
		$json = parent::json();
		global $entr_recommended_plugins;
		...
		$show_recommended_plugins = get_option( 'entr_show_recommended_plugins' );

		foreach ( $entr_recommended_plugins as $slug => $plugin_opt ) {
			...
			$entr_recommended_plugin['url']    = $this->create_action_link( $active['needs'], $slug );
			if ( $active['needs'] !== 'install' && $active['status'] ) {
				$entr_recommended_plugin['class'] = 'active';
			} else {
				$entr_recommended_plugin['class'] = '';
			}

			switch ( $active['needs'] ) {
				case 'install':
					$entr_recommended_plugin['button_class'] = 'install-now button';
					$entr_recommended_plugin['button_label'] = $install_button_label;
					break;
				case 'activate':
					$entr_recommended_plugin['button_class'] = 'activate-now button button-primary';
					$entr_recommended_plugin['button_label'] = $activate_button_label;
					break;
				case 'deactivate':
					$entr_recommended_plugin['button_class'] = 'deactivate-now button';
					$entr_recommended_plugin['button_label'] = $deactivate_button_label;
					break;
			}
			$info   = $this->call_plugin_api( $slug );
			$entr_recommended_plugin['id'] = $slug;
			$entr_recommended_plugin['plugin_slug'] = $slug;

			if ( ! empty( $plugin_opt['description'] ) ) {
				$entr_recommended_plugin['description'] = $plugin_opt['description'];
			} else {
				$entr_recommended_plugin['description'] = $info->short_description;
			}

			$entr_recommended_plugin['title'] = $info->name;

			$customize_plugins[] = $entr_recommended_plugin;
				...
				<div class="recomended-actions_container" id="plugin-filter">
					<# if( data.recommended_plugins.length > 0 ){ #>
						<# for (action in data.recommended_plugins) { #>
							<div class="entr-recommeded-actions-container entr-recommended-plugins" data-index="{{ data.recommended_plugins[action].index }}">
								<# if( !data.recommended_plugins[action].check ){ #>
									<div class="entr-recommeded-actions">
