<?php
/**
 * Entr Customizer Notification System.
 *
 * @package Entr
 */
...
/**
 * Entr Customizer Notify Class
 */
class Entr_Customizer_Notice {
	...
	/**
	 * The single instance of Entr_Customizer_Notice
	 *
	 * @var Entr_Customizer_Notice $instance The Entr_Customizer_Notice instance.
	 */
	...
	/**
	 * The Main Entr_Customizer_Notice instance.
	 *
	 * We make sure that only one instance of Entr_Customizer_Notice exists in the memory at one time.
	 *
	 * @param array $config The configuration array.
	 */
	public static function init( $config ) {
		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Entr_Customizer_Notice ) ) {
			self::$instance = new Entr_Customizer_Notice;
	...
	public function setup_config() {
		...
		$entr_recommended_plugins = array();
		
		if ( isset( $this->recommended_plugins ) ) {
			$entr_recommended_plugins = $this->recommended_plugins;
		}
	...
	public function scripts_for_customizer() {
		...
		wp_enqueue_style( 'entr-customizer-notice', get_template_directory_uri() . '/lib/customizer/notice/css/customizer-notice.css', array() );
		...
		wp_enqueue_script( 'entr-customizer-notice', get_template_directory_uri() . '/lib/customizer/notice/js/customizer-notice.js', array( 'customize-controls' ) );
		wp_localize_script(
			'entr-customizer-notice', 'customizer_notice_data', array(
				'ajaxurl'                  => admin_url( 'admin-ajax.php' ),
				'template_directory'       => get_template_directory_uri(),
				'base_path'                => admin_url(),
				'activating_string'        => esc_html__( 'Activating', 'entr' ),
			)
		);
	...
	public function customize_register_notice( $wp_customize ) {
		...
		/**
		 * Include the Entr_Customizer_Notice_Section class.
		 */
		...
		$wp_customize->register_section_type( 'Entr_Customizer_Notice_Section' );

		$wp_customize->add_section(
			new Entr_Customizer_Notice_Section(
				$wp_customize,
	...
	public function dismiss_recommended_plugins_callback() {
		...
		if ( ! empty( $action_id ) ) {
			...
			update_option( 'entr_show_recommended_plugins', $show_recommended_plugins );
		}
